...
import DuelLeaderboard from '../duels/DuelLeaderboard';
...
const FILTERS = [
  { key: 'incoming',    label: 'Primljeni' },
  { key: 'active',      label: 'Aktivni' },
  { key: 'outgoing',    label: 'Poslani' },
  { key: 'history',     label: 'Povijest' },
  { key: 'leaderboard', label: '🏆 Ljestvica' },
];

export default function DuelsPanel({ loadBalance }) {
  ...
  const outgoing  = duels.filter(d => d.challenger_email === user?.email && d.status === 'pending');
  const active    = duels.filter(d =>
    (d.challenger_email === user?.email || d.opponent_email === user?.email) && d.status === 'accepted'
  );
  ...
  const accept = async (duel) => {
    ...
    if (res.data?.error) { toast.error(res.data.error); setProcessing(null); return; }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h2 className="text-2xl font-display font-black tracking-wide uppercase flex items-center gap-2">
            <Swords className="w-6 h-6 text-primary" /> Dueli
          </h2>
          <p className="text-muted-foreground text-sm">Pobjednik uzima cijeli pool tokena.</p>
        </div>
        <Link
          to="/dueli/statistika"
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-secondary border border-border/50 text-sm font-semibold hover:bg-secondary/80 transition-all"
        >
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
        {FILTERS.map(f => (
          ...
          >
            {f.label}
            {counts[f.key] > 0 && (
              <span className={`text-xs px-1.5 py-0.5 rounded-full font-black ${filter === f.key ? 'bg-white/20' : 'bg-muted'}`}>
                {counts[f.key]}
              </span>
            )}
          </button>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-4 text-sm mb-4">
                  ...
                  <span className="text-xs text-muted-foreground ml-auto">{moment(duel.created_date).format('DD.MM HH:mm')}</span>
                </div>

                {duel.message && (
                  <p className="text-xs italic text-muted-foreground bg-secondary/50 rounded-xl px-3 py-2 mb-3">"{duel.message}"</p>
                )}
                ...
                {filter === 'incoming' && duel.status === 'pending' && (
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => accept(duel)} disabled={processing === duel.id}
                    ...
                    <button onClick={() => decline(duel)} disabled={processing === duel.id}
                ...
                {filter === 'active' && (
                  <button onClick={() => resolve(duel)} disabled={processing === duel.id}
